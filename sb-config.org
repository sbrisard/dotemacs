...
We define some variables which can be custom-set

  - =sb-TeX-pdf-viewer-command= full path to the PDF viewer executable
  - =sb-TeX-pdf-viewer-options= PDF viewer command line options (synctex and such).
...
We use a custom =:set= function for the =sb-TeX-pdf-viewer-options= variable, in order to update =TeX-view-program-list=, which depends on it.

#+BEGIN_SRC emacs-lisp
  (defun sb-set-TeX-pdf-viewer-options (symbol value)
    "The function to be called when modifying `sb-TeX-pdf-viewer-options'
  ...
  `TeX-view-program-list', taking care of duplicate entries."
    (progn (setq TeX-view-program-list
                 (cons (list "SB PDF viewer" (concat "%(sb-pdf-viewer) " value))
  ...
  "
    ...
    :set 'sb-set-TeX-pdf-viewer-options)

  (add-to-list 'TeX-expand-list
               '("%(sb-pdf-viewer)" (lambda () (format sb-TeX-pdf-viewer-command))))
