...
#+BEGIN_SRC emacs-lisp
  ...
  (when sb-windows-p
    (defconst sb-SumatraPDF-options "-reuse-instance -forward-search %b %n %o"
      "Options to be passed to the SumatraPDF viewer.")
    (defcustom sb-SumatraPDF-executable "C:\\opt\\SumatraPDF-3.0\\SumatraPDF.exe"
      "Full path to the SumatraPDF executable."
      :type 'string
      :group 'sb
      :tag "SumatraPDF executable")
    (add-to-list 'TeX-view-program-list
		 `("SumatraPDF"
		   (,(concat sb-SumatraPDF-executable " " sb-SumatraPDF-options))
		   ,sb-SumatraPDF-executable)))

  (put 'TeX-view-program-list 'variable-comment
  ...
  ;; This does not work
  ;; (put 'TeX-view-program-selection 'standard-value
  ;;      '((output-pdf "SB PDF viewer")))
  (custom-add-to-group 'sb 'TeX-view-program-selection 'custom-variable)
#+END_SRC

To do :

  - Update TeX-view-program-list each time sb-SumatraPDF-executable is modified.
  - Define SumatraPDF as viewer for output-pdf. First remove previous entries
