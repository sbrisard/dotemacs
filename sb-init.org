...
a standard message recording the date and time. This ought to be
automated (hooks?).

#+BEGIN_SRC emacs-lisp
  (defun sb-magit-stage-all-and-commit ()
    (interactive)
    (magit-shell-command (concat "git commit -a -m \""
                                 (format-time-string "%d/%m/%Y %H:%M")
                                 "\"")))

  (define-key sb-map (kbd "C") 'sb-magit-stage-all-and-commit)
#+END_SRC
