...
** A_FAIRE Insert default commit messages

My notes are version controlled. However, I find it difficult to enclose a meaningful commit message on each commit. So I end up using a standard message recording the date and time. This ought to be automated (hooks?).
